...
export function PricePrediction({
  ...
  async function generatePrediction() {
    ...
    if (daysUntilTravel < 30) {
      predictedChange += 0.1; // +10% in last month
    ...
    // Add some randomness
    predictedChange += (Math.random() - 0.5) * 0.1;

    const predictedPrice = currentPrice * (1 + predictedChange);
    const confidence = Math.min(
      0.95,
      Math.max(0.6, 0.85 - Math.abs(predictedChange)),
    );
    ...
    if (trend === "up" && confidence > 0.75) {
      ...
      recommendation =
        "Prices are stable. Good time to book if you find a convenient option.";
    }
    ...
    for (let i = 30; i >= 0; i -= 5) {
      ...
      const historicalPrice = currentPrice * (1 + (Math.random() - 0.5) * 0.1);
      priceHistory.push({
  ...
  return (
    <Card className="p-4 bg-gradient-to-br from-white to-sky-50 border-sky-200">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
        ...
        <span>
          Best time to book:{" "}
          <strong className="text-gray-900">{prediction.bestTimeToBook}</strong>
        </span>
      </div>
        ...
        <div className="h-24 flex items-end gap-1">
          {prediction.priceHistory.map((point, index) => {
            const maxPrice = Math.max(
              ...prediction.priceHistory.map((p) => p.price),
            );
            const minPrice = Math.min(
              ...prediction.priceHistory.map((p) => p.price),
            );
            const height =
              ((point.price - minPrice) / (maxPrice - minPrice + 1)) * 100;
            return (
              <div
                key={index}
